...
const index = ({url}) => {
    ...
    const getApplication = async () =>{
      try {
        const getSingleApp = await axios.post(`${url}/application/getsingleapplication/${id}`);
  ...
  return (
   ...
   <div className='border min-h-screen rounded-sm px-5 xlg:px-10 flex-1 xlg:w-[50%]'>
        ...
        <Link to={'/listjobs'} onClick={() => deleteJobPost(id)} className='px-8 py-[13px] bg-red-500 hover:bg-red-700 duration-200 text-white rounded-md sticky bottom-3 flex my-5 text-center w-full items-center justify-center' >Deleted</Link>
          </motion.div>
          )
         })}
    </div>
